...
v3 的主代理采用 [Operating Partner](operating-partner.md) 责任模型；先作一次资本分配，沿用 DIRECT/LIGHT/FULL，再进入本架构。它不增加权限，不替代领域推理、Human 决策或 ChatGPT 独立性。
...
| EXECUTING / REPAIRING / REVERIFYING / CHATGPT_REVIEW_FAIL | STOP_LOSS | 同根因连续两次实质修复无效或原 anti-patch 信号 |
| STOP_LOSS | DIRECTION_REASSESSMENT | 保留历史，重新检查目标、假设、策略与证据；不机械第三次 patch |
