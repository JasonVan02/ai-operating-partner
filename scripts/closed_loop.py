...
def gate(spec):
    ...
    # Optional v3 execution metadata; the V1 wire protocol and legacy inputs stay valid.
    # State belongs to the reviewed Evidence, not to an unreviewed gate override.
    capital_state = evidence.get("capital_state", "CONTINUE")
    require(isinstance(capital_state, str) and capital_state in {"CONTINUE", "STOP_LOSS", "REASSESSED"},
            "Invalid capital_state")
    require(capital_state != "STOP_LOSS", "STOP_LOSS requires direction reassessment before completion")
    ...
    if capital_state == "REASSESSED":
        refs = evidence.get("reassessment_evidence")
        require(isinstance(refs, list) and refs and all(nonempty(ref) and ref in registry for ref in refs),
                "REASSESSED requires current reviewed reassessment artifacts")
        require(len(refs) == len(set(refs)), "Duplicate reassessment evidence reference")
        require(all(bool(registry[ref][1].strip()) for ref in refs), "Reassessment evidence must not be empty")
