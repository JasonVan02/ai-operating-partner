...
资本节约不能豁免本 Gate。`STOP_LOSS` 未解除时禁止完成；有可选 v3 `capital_state` 元数据时，CLI 检查它及当前已审的 `reassessment_evidence`，见 [stop loss](stop-loss.md)。这是执行状态，不扩展 Review 枚举。真实条件通过后停止可选优化；新缺陷或变化才重开相关检查。资本复盘见 [capital ledger](capital-ledger.md)，不引入第二个验收门禁。
